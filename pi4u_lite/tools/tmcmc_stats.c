...
double Objlogp(double x, double *fj, int fn, double pj, double tol)
{
    ...
    for (i = 0; i < fn; i++)
    {
        ...
        //    printf("weight[%i] = %e, x=%e, pj=%e, fj=%e, fjmax=%e \n",i, weight[i], x, pj, fj[i], fjmax );
    }
    
    double sum_weight = compute_sum(weight, fn);
    
    //printf("weight = %e, sum_weight=%e, x=%e, pj=%e, fj=%e, fjmax=%e \n",weight[i], sum_weight, x, pj, fj[i], fjmax );
...
int fminsearch(double *fj, int fn, double pj, double tol, double *xmin, double *fmin)
{
    ...
    gsl_vector_set_all (ss, 0.001); /* input?  0.000001 */
...
void calculate_statistics(double flc[], int n, int nselections, int gen, unsigned int sel[])
{
    ...
    p[j] = xmin;
    printf("else p[%d]=%e, p[%d]=%e, n\ ", gen, p[gen], j, p[j] );
    CoefVar[j] = fmin;
