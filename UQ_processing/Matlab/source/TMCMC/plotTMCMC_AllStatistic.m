...
function plotTMCMC_AllStatistic(mydata,param,bounds,names,groundTruth,dump,KDEbins,Nbins,GenId,bSynthetic)
...
for i = 1:N
    for j = 1:N
        ...
        if( i==j)  % diagonal -> marginal
            ...
            % error bars
            % errorbar(best(parId),varData(parId),'kx','Linewidth',1.1);
...
if(dump)
    ...
    %    set(gcf, 'Position', [100, 100, 1049, 895]);
    %    set(gcf, 'Position', [500, 500, 1049, 895]);
    
    % print(gcf, 'OutputPlot','-djpeg')
    set(gcf,'PaperPositionMode','auto')
    % print('PosteriorPDF','-djpeg')
    % print('PosteriorPDF','-dpdf')
    % set(gcf, 'Color', 'None')
