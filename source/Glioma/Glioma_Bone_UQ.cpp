...
Glioma_Bone_UQ::Glioma_Bone_UQ(int argc, const char ** argv): parser(argc, argv)
{
...
void Glioma_Bone_UQ::_ic_Vertebra(Grid<W,B>& grid)
{
...
#ifdef Template23
    printf("Reading anatomy from: /cluster/home/mavt/lipkovaj/GliomaAdvance/UQ_Section/source/Anatmoy/Bone/Template/vertebra_23.dat \n");
            ...
            for(int iy=0; iy<B::sizeY; iy++)
                for(int ix=0; ix<B::sizeX; ix++)
                {
                    ...
                    if ( (mappedBoneX >= 0 && mappedBoneX < boneSizeX) && (mappedBoneY >= 0 && mappedBoneY < boneSizeY) && (mappedBoneZ >= 0 && mappedBoneZ < boneSizeZ) )
                    {
                        ...
                        block(ix,iy,iz).bmd  = (bmd > 1.) ? bmd : 0.;   // remove background signal, BMD [HU]
                        ...
                        if ((psi < -1)&& ( bmd > 1. ))		// we are in tumor
                            block(ix,iy,iz).phi = 1.0;
                        else if(( (-1 <= psi) && (psi <= 1) )&& ( bmd > 1. ))
...
void Glioma_Bone_UQ::_readInTumorPosition(vector<Real>& tumorIC )
{
...
void Glioma_Bone_UQ::_reactionDiffusionStep(BoundaryInfo* boundaryInfo, const int nParallelGranularity, const Real Dw, const Real Dg, const Real rho, double dt)
{
...
void Glioma_Bone_UQ:: _dump(int counter)
{
...
#ifdef Template16
        sprintf(filename,"Spine_v16_Data%04d", counter);
#else
        sprintf(filename,"Spine_v23_Data%04d", counter);
#endif
        
        IO_VTKNative3D<W,B, 7,0 > vtkdumper;
...
void Glioma_Bone_UQ::_dumpUQoutput(Grid<W,B>& grid)
{
...
void Glioma_Bone_UQ::run()
{
    ...
    if(bVerbose)  printf("Dw=%e, Dg=%e, dt= %f, rho=%f , h=%f\n", Dw, Dg, dt,rho,h);
