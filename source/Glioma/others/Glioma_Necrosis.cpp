...
Glioma_Necrosis::Glioma_Necrosis(int argc, const char ** argv): parser(argc, argv)
{
    bVerbose = parser("-verbose").asBool();
    
    if(bVerbose) printf("////////////////////////////////////////////////////////////////////////////////\n");
    if(bVerbose) printf("//////////////////             Glioma + Necrosis            ////////////////\n");
    if(bVerbose) printf("////////////////////////////////////////////////////////////////////////////////\n");
...
Glioma_Necrosis::~Glioma_Necrosis()
{
...
void Glioma_Necrosis:: _ic_SubjectBrain(Grid<W,B>& grid)
{
...
void Glioma_Necrosis::_ic_PatientCase(Grid<W,B>& grid)
{
    
#if !defined(Patient01) && !defined(Patient07) && !defined(Patient11) && !defined(Patient22)
#define Patient01
#endif
...
void Glioma_Necrosis::_selectBrainWebAnatomy(vector<float>& GreyTissueData, vector<float>& WhiteTissueData, vector<float> & CsfData, int dataSize)
{
    FILE * fp;
...
void Glioma_Necrosis::_readInBrainWebAnatomy(vector<float>& tissue, FILE* fp, int DataSize, int threshold )
{
    typedef uint8_t brainWebType;
...
void Glioma_Necrosis::_readInTumorPosition(vector<Real>& tumorIC )
{
...
void Glioma_Necrosis::_reactionDiffusionNecrosisStep(BoundaryInfo* boundaryInfo, const int nParallelGranularity, const Real Dw, const Real Dg, const Real rho, const Real gamma, double dt)
{
    
    vector<BlockInfo> vInfo				= grid->getBlocksInfo();
    const BlockCollection<B>& collecton = grid->getBlockCollection();
    
    Glioma_ReactionDiffusionNecrosis_Operator<_DIM>  rhs(Dw,Dg,rho,gamma);
    TimeUpdate                               <_DIM>  updateTime(dt);
    
    blockProcessing.pipeline_process(vInfo, collecton, *boundaryInfo, rhs);
    BlockProcessing::process(vInfo, collecton, updateTime, nParallelGranularity);
}


#pragma mark DumpingOutput
void Glioma_Necrosis:: _dump(int counter)
{
    ...
    if (parser("-vtk").asBool())
    {
        ...
        if( _DIM == 2)
        {
            IO_VTKNative<W,B, 4,0 > vtkdumper2;
...
void Glioma_Necrosis::run()
{
    ...
    while (t <= tend)
    {
        ...
        _reactionDiffusionNecrosisStep(boundaryInfo, nParallelGranularity, Dw, Dg, rho, gamma, dt);
